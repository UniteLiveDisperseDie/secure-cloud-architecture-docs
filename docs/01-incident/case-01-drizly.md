...
2020년 7월, 미국 최대의 주류 배달 플랫폼인 **Drizly**에서 약 **250만 명**의 고객 정보가 유출되는 대규모 보안 사고가 발생했습니다. 이 사건은 단순한 데이터 유출을 넘어, **2018년 해커톤을 위해 임시로 부여했던 권한이 2년 동안 회수되지 않고 방치**되는 등 클라우드 환경에서 관리적 보안의 부재가 기업에 어떤 치명적인 영향을 미치는지 보여주는 대표적인 사례입니다.
...
- **취약점:** 타 사이트 유출 정보를 재사용했으며, 패스워드는 **특수문자조차 없는 단 7자리**였습니다. 가장 결정적으로 **MFA(2단계 인증)를 활성화하지 않아** 아이디와 비밀번호만으로 즉시 탈취당했습니다.

### 2. 권한 악용 및 탐색

계정을 장악한 공격자는 해당 계정이 가진 권한을 탐색하기 시작했습니다.

- 2018년 4월, 단 하루의 해커톤 참가를 위해 일시적으로 부여했던 저장소 접근 권한이 **해커톤 종료 후에도 회수되지 않고 2년 뒤인 2020년까지 유지**되고 있었습니다.
...
- 이를 통해 고객 정보가 담긴 **AWS RDS**에 접속하여 약 **250만 명**의 사용자 레코드를 자신의 서버로 전송하며 공격을 마무리했습니다.

---

## 3. 대응 방안
### 1. 불필요한 권한 방치 및 과잉 권한
...
- **AWS Secrets Manager & IAM Roles**
    ...
    - **IAM Roles for EC2/Lambda:** 가능하면 Access Key 자체를 생성하지 않는 것이 최선입니다. 애플리케이션에 IAM 역할을 직접 부여하여 **임시 자격 증명**을 사용하게 함으로써 코드 저장소에 유출될 키 자체가 존재하지 않게 만듭니다.

### 4. 실시간 이상 행위 탐지 및 대응 실패
...
- **Amazon GuardDuty & AWS Config & AWS CloudTrail**
    ...
    - **AWS Config:** 리소스의 설정 변경을 실시간으로 감시합니다. 예를 들어, 보안 그룹이 `0.0.0.0/0`으로 변경되는 즉시 이를 감지하고 알림을 보내거나 이전 설정으로 자동 복구하도록 설정할 수 있습니다.
...
| **날짜** | **주요 사건** |
|---------|-------------|
| 2018년 4월 | 단 하루의 해커톤을 위해 임원에게 GitHub 저장소 접근 권한 부여 |
| 2020년 7월 | 크리덴셜 스터핑으로 임원 계정 탈취 및 AWS RDS 내 250만 명 정보 유출 |
| 2020년 7월 28일 | Drizly, 보안 사고 발생에 대한 공식 성명 발표 |
| 2021년 2월 | Uber가 11억 달러(약 1.4조 원)에 Drizly 인수 |
| 2022년 10월 24일 | 미국 연방거래위원회(FTC), Drizly의 보안 실태 조사 시작 |
| 2023년 1월 10일 | 불필요 데이터 파기 명령 및 **CEO 개인에게 보안 책임 부여**라는 이례적 판결 |
| 2024년 1월 15일 | Uber, 리스크 및 비즈니스 판단 하에 Drizly 서비스 종료 발표 |
| 2024년 3월 | Drizly 서비스 공식 종료 및 조직 해체 |
