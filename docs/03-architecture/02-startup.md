...
- **설정 방식**: ECR 접근 인증을 세 가지 역할로 구성합니다.
  ...
  - **GitHub Actions OIDC Role**: GitHub Actions에서 ECR로 이미지를 push할 때는 Access Key 대신 OIDC 기반 IAM Role을 사용합니다. 별도 IAM Role 및 Trust Policy 설정이 필요하며, 이 부분은 직접 구현해야 합니다.
